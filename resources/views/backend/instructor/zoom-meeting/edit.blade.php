                ...
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-0 p-0">
                        ...
                        <li class="breadcrumb-item"><a href="{{ route('instructor.zoom-meeting.index') }}">All Zoom
                                Meetings</a></li>
                        <li class="breadcrumb-item active" aria-current="page">Edit Zoom Meeting</li>
                    </ol>
                </nav>
            </div>
        </div>
        ...
        <div class="card">
            <div class="card-body p-4">
                <div style="display: flex; align-items:center; justify-content:space-between">
                    <h5 class="mb-4">Edit Zoom Meeting</h5>
                    <a href="{{ route('instructor.zoom-meeting.index') }}" class="btn btn-primary">Back</a>
                </div>

                <form class="row g-3" method="post" action="{{ route('instructor.zoom-meeting.update', $meeting->id) }}">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="col-md-12">
                        ...
                        <input type="text" class="form-control" name="title" id="title"
                            placeholder="Enter meeting title" value="{{ old('title', $meeting->title) }}" required>
                    </div>
                        ...
                        <select class="form-select" name="course_id" id="course_id" required>
                            ...
                            @foreach ($courses as $course)
                                <option value="{{ $course->id }}"
                                    {{ old('course_id', $meeting->course_id) == $course->id ? 'selected' : '' }}>
                                    {{ $course->course_name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-12">
                        ...
                        <input type="url" class="form-control" name="meeting_url" id="meeting_url"
                            placeholder="https://zoom.us/j/..." value="{{ old('meeting_url', $meeting->meeting_url) }}"
                            required>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="number" class="form-control" name="duration" id="duration"
                            placeholder="Enter duration in minutes" value="{{ old('duration', $meeting->duration) }}"
                            min="1" required>
                    </div>

                    <div class="col-md-12">
                        <div class="d-md-flex d-grid align-items-center gap-3">
                            <button type="submit" class="btn btn-primary px-4 w-100">Update Zoom Meeting</button>
                        </div>
                    </div>
                </form>
            </div>
